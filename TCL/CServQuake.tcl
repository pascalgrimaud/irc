#
# CServQuake.tcl par Ibu <ibu_lordaeron@yahoo.fr>
#
# Aidé par :
...
set cservauth(cserv) "Q@CServe.quakenet.org"
set cservauth(cserv2) "L@lightweight.quakenet.org"

# ID-Nick de votre EggDrop
set cservauth(idnick) "NT_Bot"
...
putlog "\002CServQuake.tcl.tcl\002 - par \002Ibu\002 14<ibu_lordaeron@yahoo.fr>"
...
#    putdcc $idx "  .deauth <#channel> (pour se deauth)"
    ...
    putdcc $idx "  CServ2  : $cservauth(cserv2)"
...
proc CServ:turn { hand idx text } {
    ...
    if { $cservauth(start) } {
        ...
        putdcc $idx "\[CServAuth\] ::: desactivé"
        putlog "#$hand# authturn -> (off)"
    } {
        set cservauth(start) 1
        putdcc $idx "\[CServAuth\] ::: activaté"
            ...
            if { [validchan $CSChkchan] && $CSChkoping != 2 } {
                if { [botonchan $CSChkchan] } {
                    if { ![botisop $CSChkchan] && $CSChkoping == 1 } {
                        putloglev 7 * "PRIVMSG $cservauth(cserv) :AUTH $cservauth(idnick) \[something\]"
                        putloglev 7 * "PRIVMSG $cservauth(cserv2) :OP $CSChkchan"
                        putserv "PRIVMSG $cservauth(cserv) :AUTH $cservauth(idnick) $CSChkpass"
                        putserv "PRIVMSG $cservauth(cserv2) :OP $CSChkchan"
                    }
                } else {
                    putloglev 7 * "PRIVMSG $cservauth(cserv) :AUTH $cservauth(idnick) \[something\]"
                    putloglev 7 * "PRIVMSG $cservauth(cserv2) :INVITE $CSChkchan"
                    putserv "PRIVMSG $cservauth(cserv) :AUTH $cservauth(idnick) $CSChkpass"
                    putserv "PRIVMSG $cservauth(cserv2) :INVITE $CSChkchan"
                }
            }
            unset CSChkchan
            unset CSChkpass
